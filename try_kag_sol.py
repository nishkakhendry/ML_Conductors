...
train = pd.read_csv("nomad2018-predict-transparent-conductors\\train.csv")
test = pd.read_csv("nomad2018-predict-transparent-conductors\\test.csv")
test_id=test.id
...
for dev_index, val_index in kf.split(X):
    ...
    y_pred_feen,rmsle_feen,y_pred_test_feen = runRF(dev_X, dev_y, val_X, val_y,test)
